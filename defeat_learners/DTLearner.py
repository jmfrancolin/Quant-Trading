...
class DTLearner(object):
    ...
    def get_feature_index(self, dataX, dataY):
        ...
        corr = np.corrcoef(dataX.T, dataY)[-1]

        # delete correrlation Y -> Y
        corr = np.delete(corr, -1)

        # consider the abs of the resulting array
        corr = np.abs(corr)

        # return the index of the feature with greatter corr
        return np.argmax(corr).astype(int)
